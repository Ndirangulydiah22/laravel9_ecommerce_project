  ...
  <!-- Content Wrapper. Contains page content -->
	 ...
	 <tr>
		...
		<td>{{ $item->blog_category_name_en }}</td>
		 <td>{{ $item->blog_category_name_kis }}</td>
		<td>
...
<!--   ------------ Add Blog Category Page -------- -->
 ...
 <form method="post" action="{{ route('blogcategory.store') }}" >
	...
	<div class="form-group">
		<h5>Blog Category Kiswahili <span class="text-danger">*</span></h5>
		<div class="controls">
	 <input type="text" name="blog_category_name_kis" class="form-control" >
     @error('blog_category_name_kis') 
	 <span class="text-danger">{{ $message }}</span>
	 @enderror 
	  </div>
	</div>
