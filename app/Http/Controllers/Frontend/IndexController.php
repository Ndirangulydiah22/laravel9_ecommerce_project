...
class IndexController extends Controller
{
    ...
    public function ProductDetails($id,$slug){
		...
		$color_kis = $product->product_color_kis;
		$product_color_kis = explode(',', $color_kis);

		$size_en = $product->product_size_en;
		$product_size_en = explode(',', $size_en);

		$size_kis = $product->product_size_kis;
		$product_size_kis = explode(',', $size_kis);

        $multiImag = MultiImg::where('product_id',$id)->get();
        $cat_id = $product->category_id;
		$relatedProduct = Product::where('category_id',$cat_id)->where('id','!=',$id)->orderBy('id','DESC')->get();
	 	return view('frontend.product.product_details',compact('product','multiImag','product_color_en','product_color_kis','product_size_en','product_size_kis','relatedProduct'));
	

    }
    public function TagWiseProduct($tag){
		$products = Product::where('status',1)->where('product_tags_en',$tag)->where('product_tags_kis',$tag)->orderBy('id','DESC')->paginate(3);
        $categories = Category::orderBy('category_name_en','ASC')->get();
